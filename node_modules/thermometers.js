    ...
    thermoLab.on('connect', function(t, data) {
        ...
        t.on('tData', function(data){
            ...
            data.forEach(function(entry) {
                console.log('Logging', entry.t, '*C against', entry.device);
...
ThermometerLogger.prototype = {
    ...
    getSensorLog: function(deviceId, duration, callback) {
        this.db.all('SELECT s.device_id as device, l.value as t, l.timestamp as time FROM temperature_log l JOIN temperature_sensors s ON s.ROWID = l.sensor_id WHERE l.timestamp > ? AND s.device_id = ?', new Date().getTime() - duration, deviceId, function(err, rows){
            callback(rows);
        });
    }
};