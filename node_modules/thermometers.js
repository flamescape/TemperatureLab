...
ThermometerLogger.prototype = {
    ...
    getSensorLog: function(sensorId, duration, callback) {
        this.db.all('SELECT s.device_id as device, l.value as t, l.timestamp as time FROM temperature_log l JOIN temperature_sensors s ON s.ROWID = l.sensor_id WHERE l.timestamp > ?', new Date().getTime() - duration, function(err, rows){
            callback(rows);
        });
    }
};